...
func main() {
	...
	runs, err := qaseService.GetLatestTestRuns(project, dailyRunTitle, from, 1)
	if err != nil {
		logrus.Fatalf("Error getting latest '%s' run: %v", dailyRunTitle, err)
	}

	if len(runs) != 1 {
		logrus.Fatalf("Resulting run slice has lenght %d, expected 1.", len(runs))
	}
