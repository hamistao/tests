...
const (
	// weeklyRunTitle is the title of the weekly Test Run on Qase that hold the target results.
	weeklyRunTitle = "Staging PIT Weekly Test Run"
	// dailyRunTitle is the title of the daily Test Run on Qase that hold the target results.
	dailyRunTitle = "Staging PIT Daily Test Run"
...
func main() {
	...
	weeklyRunSlice, err := qaseService.GetLatestTestRuns(project, weeklyRunTitle, from, 3) // There are three weekly runs for different rancher versions.
	if err != nil {
		logrus.Fatalf("error getting '%s' runs for the past week: %v", weeklyRunTitle, err)
	}
	...
	dailyRuns, err := qaseService.GetLatestTestRuns(project, dailyRunTitle, from, 7)
	if err != nil {
		logrus.Fatalf("error getting '%s' runs for the past week: %v", dailyRunTitle, err)
	}
